...
7. Добро пожаловать https://gist.github.com/kanat3/fed664284e444bb549bf7ee62cb39853 (обновила)
...
2. Обучила нейронку, которая распознает пол, результат +/- нормальный

#### 10.11.2020 - 11.11.2020
1. Т.к меня выкинул коллаб во время обучения нейронки, которая распознает пол, то графиков у меня нет(( Однако сохранены веса на первом этапе её обучения и как раз на них 
выдается точность ~ 93% (22 эпохи, скорость обучения - 0,001)
2. Далее я попыталась заново обучить нейронку на той же модели, и хмхмхм почему-то ничего не обучается
3. Далее я просто взяла последний чекпоинт и дообучила его до наилучшей точности (16 эпох, скорость обучения 0,01 на том же оптимизаторе)
В итоге точность на тестовых данных ~  98%. Но это все на русском датасете
4. Полностью перевела train английского датасета в спектрограммы и залила на диск
5. И немного поработала со статьей 
6. Эпилепсия какая-то на определении возраста :/ https://gist.github.com/kanat3/76a0a139b06989d819621f08f4f0fe98
#### 18.11.2020
1. Эту штуку все еще надо вести?(((
2. Обучение модели возраста остановилось на 50% точности валидации
...
4. Можно сохранять состояние оптимизатора для такого типа файла? хмхмхмхмхм
